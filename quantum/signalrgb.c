...
void led_streaming(uint8_t *data) //Stream data from HID Packets to Keyboard.
{
    ...
    for (uint8_t i = 0; i < numberofleds; i++)
    {
      ...
      //if ( ((index + i) == CAPS_LOCK_INDEX && host_keyboard_led_state().caps_lock) || ((index + i) == NUM_LOCK_INDEX && host_keyboard_led_state().num_lock) || ((index + i) == SCROLL_LOCK_LED_INDEX && host_keyboard_led_state().scroll_lock))   {
